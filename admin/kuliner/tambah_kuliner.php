...
// Proses penambahan kuliner baru
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Memeriksa apakah file foto telah diunggah
    if (!empty($_FILES['photo']['name'])) {
        // Memeriksa jenis file foto
        $fotoType = $_FILES['photo']['type'];
        $allowedFotoTypes = ['image/jpeg', 'image/png', 'image/gif'];
        if (!in_array($fotoType, $allowedFotoTypes)) {
        ...
        if (move_uploaded_file($_FILES["photo"]["tmp_name"], $targetFileCover)) {
            ...
            $query = "INSERT INTO kuliner (Nama, Lokasi, Kategori, Deskripsi, Rating, Photo) 
        ...
        // Jika pengguna tidak memilih file foto
        echo '<script>alert("Silakan pilih file foto!");</script>';
        exit();
    }
}
?>
...
<div class="main-content">
  <div class="container-fluid">
    <h2 class="mb-4"><i class="bi bi-utensils"></i> Tambah Kuliner</h2>
...
<button type="submit" class="btn btn-primary"><i class="fas fa-plus"></i> Tambah Kuliner</button>
</div>
</div>
